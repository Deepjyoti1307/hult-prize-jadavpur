...
import { Search, MoreVertical, Phone, Video, Image as ImageIcon, Mic, Send, CheckCheck, Smile, MessageSquare, AlertCircle } from 'lucide-react';
...
export default function MessagesView({
    emptyStateSubtitle = 'Start chatting to connect.',
    ...
    return (
        <div className="p-4 md:p-8 pt-20 relative z-10 w-full h-screen mx-auto flex flex-col gap-4 max-w-[1600px]">
            {chatError && (
                <div className="flex items-start gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-red-200">
                    <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />
                    <div className="text-sm">
                        <p className="font-medium">Chat failed to load</p>
                        <p className="text-red-200/80 mt-1">{chatError}</p>
                        <a
                            href="https://console.firebase.google.com/project/tarang-5bace/firestore/indexes"
                            target="_blank"
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                    {filteredConversations.length === 0 ? (
                        <div className="text-center py-12 text-white/40 px-4">
                            ...
                            <p className="text-sm mt-1">{emptyStateSubtitle}</p>
                            {emptyStateActionLabel && emptyStateActionHref && (
                                <Link
                                    href={emptyStateActionHref}
                                    className="inline-block mt-4 px-4 py-2 rounded-xl bg-accent/20 text-accent border border-accent/30 text-sm font-medium hover:bg-accent/30 transition-colors"
                                >
                                    {emptyStateActionLabel}
                                </Link>
                            )}
